...
func NewHandler(name string, addr string, stateDir string, debug bool, verbose bool, socketGroup string, services ...types.ServiceType) (*Handler, error) {
	...
	for _, serviceType := range services {
		...
		case types.MicroCloud:
			service, err = NewCloudService(name, addr, stateDir, verbose, debug, socketGroup)
