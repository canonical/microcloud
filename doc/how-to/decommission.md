...
Refer to the {ref}`LXD decommissioning guide <lxd:howto-decommission>` and the {doc}`MicroCeph guide to removing disks <microceph:how-to/remove-disk>` for related information.
...
Instances, profiles, and custom volumes are scoped by {ref}`project <lxd:projects>`.
...
Delete data from any external systems that you used to monitor {ref}`LXD events <lxd:logs_loki>`, {ref}`LXD metrics <lxd:metrics>`, or {doc}`Ceph logging <microceph:explanation/security/security-overview>`, such as [Loki](https://grafana.com/oss/loki/), [Prometheus](https://prometheus.io/), or [Grafana](https://grafana.com/).
...
To {doc}`remove MicroCeph OSDs <microceph:how-to/remove-disk>`, list all disks, then remove each one:
...
For clusters {ref}`configured with OIDC <lxd:authentication-openid>`, consult your OIDC identity provider for the steps to remove any data associated with your profile.
