...
func sendRequestToClusterManager(sh *service.Handler, joinToken *api.ClusterMemberJoinToken) error {
	client, publicKey, err := NewClusterManagerClient(sh, joinToken.Addresses[0], joinToken.Fingerprint)
	if err != nil {
		return err
	}
...
func NewClusterManagerClient(sh *service.Handler, address string, expectedFingerprint string) (*http.Client, string, error) {
	client := &http.Client{}
	...
	if !strings.EqualFold(remoteFingerprint, expectedFingerprint) {
		return nil, "", fmt.Errorf("Invalid cluster manager certificate fingerprint")
	}
...
func clusterManagerDelete(sh *service.Handler) func(state state.State, r *http.Request) response.Response {
	return func(state state.State, r *http.Request) response.Response {
		...
		client, _, err := NewClusterManagerClient(sh, clusterManager.Addresses, clusterManager.Fingerprint)
		if err != nil {
			return response.InternalError(err)
		}
