...
func serviceTokensPost(s state.State, r *http.Request) response.Response {
	...
	fs := s.FileSystem()
	if fs == nil {
		return response.InternalError(fmt.Errorf("State filesystem is nil"))
	}

	sh, err := service.NewHandler(s.Name(), req.ClusterAddress, fs.StateDir, false, false, "", types.ServiceType(serviceType))
	if err != nil {
		return response.SmartError(err)
	}
