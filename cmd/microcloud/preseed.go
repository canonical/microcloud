...
func (p *Preseed) validate(name string, bootstrap bool, systems []*types.System) error {
	...
	for _, system := range systems {
		...
		// Some checks can only be performed on the initiator once a view of all joining systems is available.
		initiator := p.isInitiator(system.Name, system.Address)

		if system.UplinkInterface != "" {
			uplinkCount++

			if p.OVN.IPv4Gateway == "" && p.OVN.IPv6Gateway == "" && bootstrap && initiator {
...
func (p *Preseed) Parse(s *service.Handler, c *initConfig, installedServices map[types.ServiceType]string, expectedSystems int) (map[string]InitSystem, error) {
	...
	if !initiator {
		// Validate the joiner's preseed configuration.
	...
	// Validate the full preseed configuration.
