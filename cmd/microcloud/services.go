...
func (c *cmdServiceList) Run(cmd *cobra.Command, args []string) error {
	...
	s, err := service.NewHandler(status.Name, status.Address.Addr().String(), c.common.FlagMicroCloudDir, c.common.FlagLogDebug, c.common.FlagLogVerbose, c.common.FlagSocketGroup, services...)
	if err != nil {
		return err
	}
...
func (c *cmdServiceAdd) Run(cmd *cobra.Command, args []string) error {
	...
	// Instantiate a handler for the services.
	s, err := service.NewHandler(cfg.name, cfg.address, c.common.FlagMicroCloudDir, c.common.FlagLogDebug, c.common.FlagLogVerbose, c.common.FlagSocketGroup, services...)
	if err != nil {
		return err
	}
