...
func (c *initConfig) askRemotePool(sh *service.Handler) error {
	...
	if len(askSystemsRemote) != 0 {
		...
		if availableDiskCount == 0 && totalExistingClusterDisks == 0 {
		...
		if totalExistingClusterDisks > 0 && wantsDisks {
			fmt.Println()
		...
		if availableDiskCount > 0 && wantsDisks {
			err = c.askRetry("Change disk selection?", func() error {
				...
				if len(targetDisks) == 0 && totalExistingClusterDisks == 0 {
				...
				for target, disks := range existingClusterDisks {
					_, ok := mergedDisks[target]
					if !ok {
						mergedDisks[target] = []string{}
					}

					mergedDisks[target] = append(mergedDisks[target], disks...)
				}
		...
		if len(selectedDisks) == 0 && totalExistingClusterDisks == 0 {
