...
func (c *cmdClusterManagerShow) run(_ *cobra.Command, _ []string) error {
	...
	err = apiClient.QueryStruct(context.Background(), "GET", "1.0", api.NewURL().Path("cluster-managers", "default"), nil, &clusterManager)
	if err != nil {
		return err
	}
...
func (c *cmdClusterManagerDelete) run(_ *cobra.Command, _ []string) error {
	...
	err = apiClient.QueryStruct(context.Background(), "DELETE", "1.0", api.NewURL().Path("cluster-managers", "default"), payload, nil)
	if err != nil {
		return err
	}
...
func (c *cmdClusterManagerGet) run(_ *cobra.Command, args []string) error {
	...
	case "update-interval-seconds":
		value, ok := clusterManager.ExtraConfiguration[database.UpdateIntervalSecondsKey]
		if ok {
			fmt.Printf("%s\n", value)
		}
...
func (c *cmdClusterManagerSet) run(_ *cobra.Command, args []string) error {
	...
	err = apiClient.QueryStruct(context.Background(), "PUT", "1.0", api.NewURL().Path("cluster-managers", "default"), payload, nil)
	if err != nil {
		return err
	}
