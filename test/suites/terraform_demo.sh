...
test_terraform_demo() {
  ...
  lxc exec micro1 -- lxc launch ubuntu:24.04 test-container

  # Wait for container to be ready and then delete it
  lxc exec micro1 -- sh -c "$(declare -f waitInstanceReady); waitInstanceReady test-container"
  lxc exec micro1 -- lxc delete test-container --force
