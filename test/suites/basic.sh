...
test_interactive() {
  ...
  if check_api_extension auth_bearer micro01; then
  ...
  for m in micro01 micro02 micro03 ; do
    lxc exec "${m}" -- sysctl -wq net.ipv6.conf.all.disable_ipv6=1
    lxc exec "${m}" -- snap disable microceph || true
    lxc exec "${m}" -- snap disable microovn || true
    lxc exec "${m}" -- snap restart microcloud
  done
 ...
 # Unset the lookup interface because we don't have multiple addresses to select from anymore.
  ...
  # Initiate a MicroCloud cluster with a short session timeout and verify both initiator and joiner receive "Session timeout exceeded".
  reset_systems 2 3 0
  unset_interactive_vars
