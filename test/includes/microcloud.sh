...
validate_system_lxd() {
    ...
    # We cannot just check for the 2 track as the change which sets user.microcloud might be in the 2/edge but not yet 2/candidate channel.
    ...
    if ! printf "%s\n2.1.0" "${microcloud_version}" | sort -C -V; then
