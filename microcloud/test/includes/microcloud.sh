...
microcloud_interactive() {
  ...
  REUSE_EXISTING_COUNT=${REUSE_EXISTING_COUNT:-0} # (number) represents the count of services we are reusing or skipping
  REUSE_EXISTING=${REUSE_EXISTING:-}              # (yes/no) input to reuse or skip any existing services. Should be unset if there are none.
