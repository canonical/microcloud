...
func (c *CmdControl) RunPreseed(cmd *cobra.Command, preseed string, init bool) error {
	...
	services, err = askMissingServices(services, optionalServices, true)
	if err != nil {
		return err
	}
	...
	if !init {
		peers, err := s.Services[types.MicroCloud].ClusterMembers()
		if err != nil {
			return err
		}
...
func (p *Preseed) validate(name string, bootstrap bool) error {
	...
	for _, system := range p.Systems {
		...
		if bootstrap && system.Name == name {
			containsLocal = true
		}

		if system.UplinkInterface != "" {
			containsUplinks = true
		}

		if len(system.Storage.Ceph) > 0 {
			containsCephStorage = true
		}
	}

	if bootstrap && !containsLocal {
	...
	if bootstrap && containsCephStorage && len(p.Storage.Ceph) > 0 && (len(p.Systems)) < 3 {
		return fmt.Errorf("At least 3 systems are required to configure distributed storage")
	}

	for _, filter := range p.Storage.Ceph {
		if filter.Find == "" {
			return fmt.Errorf("Received empty filter")
		}

		if filter.FindMax > 0 {
			if filter.FindMax < filter.FindMin || filter.FindMax < 3 {
				return fmt.Errorf("Invalid storage filter constraints find_max (%d) must be at least 3 and larger than find_min (%d)", filter.FindMax, filter.FindMin)
			}
		}
	}
...
func (p *Preseed) Parse(s *service.Handler, bootstrap bool) (map[string]InitSystem, error) {
	...
	if usingOVN {
		...
		networks, err := lxd.GetUplinkInterfaces(bootstrap, infos)
		if err != nil {
			return nil, err
		}
	...
	for peer, system := range systems {
		...
		if len(system.MicroCephDisks) > 0 || len(system.JoinConfig) > 0 || len(system.TargetStoragePools) > 0 || len(system.StoragePools) > 0 {
			continue
		}

		allResources[peer], err = s.Services[types.LXD].(*service.LXDService).GetResources(peer, system.ServerInfo.Address, system.ServerInfo.AuthSecret)
		if err != nil {
	...
	if bootstrap && len(zfsMachines)+len(directZFSMatches) > 0 && len(zfsMachines)+len(directZFSMatches) < 3 {
		return nil, fmt.Errorf("Failed to find at least 3 disks on 3 machines for local storage pool configuration")
	}
