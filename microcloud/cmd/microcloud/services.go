...
func (c *cmdServiceAdd) Run(cmd *cobra.Command, args []string) error {
	...
	cloudCluster, err := s.Services[types.MicroCloud].ClusterMembers(context.Background(), nil)
	if err != nil {
		return fmt.Errorf("Failed to inspect existing cluster: %w", err)
	}

	lxdCluster, err := s.Services[types.LXD].ClusterMembers(context.Background(), nil)
	if err != nil {
		return fmt.Errorf("Failed to inspect existing cluster: %w", err)
	}
